...
PID_FILE="$SHARED/gateway.pid"
LOG_FILE="$SHARED/gateway.log"
...
start_gateway() {
  if [[ ! -f "$ENV_FILE" ]]; then
    ...
    echo "Create it from services/techhaven-gateway/.env.example and chmod 600 it." >&2
    exit 1
  fi
  if [[ ! -f "$ENTRY" ]]; then
  ...
  if existing="$(read_pid)" && kill -0 "$existing" 2>/dev/null; then
    if is_expected_process "$existing"; then
      echo "Agent Gateway is already running (pid=$existing)"
      return
    fi
    echo "Refusing to reuse pid $existing: it is not this Agent Gateway" >&2
    exit 1
  fi
  ...
  if ! kill -0 "$pid" 2>/dev/null || ! is_expected_process "$pid"; then
    echo "Agent Gateway failed to stay running; inspect $LOG_FILE" >&2
    exit 1
  fi
  echo "Agent Gateway started (pid=$pid)"
}

stop_gateway() {
  local pid
  if ! pid="$(read_pid)"; then
    echo "Agent Gateway is not running (no valid pid file)"
    return
  fi
  ...
  if ! is_expected_process "$pid"; then
    echo "Refusing to stop pid $pid: it is not this Agent Gateway" >&2
    exit 1
  fi
  kill "$pid"
  for _ in {1..20}; do
    if ! kill -0 "$pid" 2>/dev/null; then
      rm -f "$PID_FILE"
      echo "Agent Gateway stopped"
      return
    fi
    sleep 0.5
  done
  echo "Agent Gateway did not stop within 10 seconds" >&2
  exit 1
}
...
gateway_port() {
  ...
  if [[ -f "$ENV_FILE" ]]; then
    ...
    source "$ENV_FILE"
    set +a
    port="${TECHHAVEN_GATEWAY_PORT:-3091}"
  fi
  printf '%s' "$port"
}
...
case "$ACTION" in
  ...
  status)
    ...
    if [[ -n "$pid" ]] && kill -0 "$pid" 2>/dev/null && is_expected_process "$pid"; then
      echo "Agent Gateway is running (pid=$pid)"
    else
      echo "Agent Gateway is not running"
      exit 1
    fi
    ;;
  health)
