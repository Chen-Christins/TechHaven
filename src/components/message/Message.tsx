...
export const MessageProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const removeMessage = useCallback((id: string) => {
    // 先添加淡出动画，再移除元素
    setMessages((prev) => prev.map((msg) => (msg.id === id ? { ...msg, isClosing: true } : msg)));
...
const MessageContainer: React.FC = () => {
  ...
  const getIcon = (type: MessageType) => {
    switch (type) {
      case "success":
        return (
          <span className={styles.icon} style={{ color: "#52c41a" }}>
            ✓
          </span>
        );
      case "warn":
        return (
          <span className={styles.icon} style={{ color: "#faad14" }}>
            !
          </span>
        );
      case "error":
        return (
          <span className={styles.icon} style={{ color: "#f5222d" }}>
            ✕
          </span>
        );
      case "info":
        return (
          <span className={styles.icon} style={{ color: "#1890ff" }}>
            i
          </span>
        );
    }
  ...
  return (
    <>
      {Object.entries(messagesByPosition).map(([position, positionMessages]) => (
        <div key={position} className={`${styles.container} ${styles[position as MessagePosition]}`}>
          {positionMessages.map((message) => (
            <div
              key={message.id}
              className={`${styles.message} ${styles[message.type || "info"]} ${message.isClosing ? styles.fadeOut : ""}`}
            >
