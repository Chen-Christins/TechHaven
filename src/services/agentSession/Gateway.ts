...
export function createGatewaySession(onSid: (sid: string) => void): SessionHandle {
    ...
    const writeCheckpoint = (value: string): void => {
        try {
            if (value) {
                window.sessionStorage.setItem(checkpointKey, value);
    ...
    const handle: SessionHandle = {
                ...
                try {
                    ...
                    disposeStream = client.subscribeEvents(targetSid, {
                        ...
                        onEnd: (reason) => {
                            ...
                            if (reason === "failed") {
                                emitConnectionFailure("Agent 服务连接已中断，请稍后重试。其他页面仍可正常使用。");
                            }
                        },
                    });
        ...
        cancel() {
            ...
            gatewayCreateInFlight = null;
            if (sid) {
                void client.cancel(sid).catch(() => undefined);
            }
        },
        dispose() {
            disposed = true;
