...
export class ArticleService {
    ...
    static async searchArticles(params: {
        ...
        const q: string[] = [];
        if (params.keyword) {
            q.push(`keyword=${encodeURIComponent(params.keyword)}`);
        }
        if (params.state !== undefined) {
            q.push(`state=${params.state}`);
        }
        if (params.category_id !== undefined) {
            q.push(`category_id=${params.category_id}`);
        }
        if (params.label_id !== undefined) {
            q.push(`label_id=${params.label_id}`);
        }
        if (params.channel !== undefined) {
            q.push(`channel=${params.channel}`);
        }
        if (params.user_id !== undefined) {
            q.push(`user_id=${params.user_id}`);
        }
        if (params.year_month) {
            q.push(`year_month=${encodeURIComponent(params.year_month)}`);
        }
        if (params.page) {
            q.push(`page=${params.page}`);
        }
        if (params.page_size) {
            q.push(`page_size=${params.page_size}`);
        }
