...
const PermissionManagement: React.FC = () => {
    ...
    () =>
      Array.from({ length: 100 }, (_, index) => {
        ...
        return {
          ...
          permissions: roleAssignment.roles.flatMap((roleId) => mockRoles.find((r) => r.id === roleId)?.permissions || []),
  ...
  const filteredRoles = useMemo(() => {
    return roles.filter((role) => {
      if (filters.search) {
        ...
        if (!role.name.toLowerCase().includes(searchTerm) && !role.description.toLowerCase().includes(searchTerm)) {
          return false;
        }
      }
  ...
  const currentRoles = useMemo(() => filteredRoles.slice(startIndex, endIndex), [filteredRoles, startIndex, endIndex]);
  ...
  const toggleUserStatus = (userPermission: UserPermission) => {
    ...
    setUserPermissions((prev) => prev.map((user) => (user.id === userPermission.id ? { ...user, status: newStatus } : user)));
  };
  ...
  return (
    <div className={styles.permissionManagement}>
      ...
      <div className={styles.tabContainer}>
        <button className={`${styles.tabButton} ${activeTab === "roles" ? styles.active : ""}`} onClick={() => setActiveTab("roles")}>
          <FaUserTag />
          角色管理
        </button>
      ...
      <div className={styles.tableContainer}>
        <div className={styles.tableHeader}>
          ...
          <div className={styles.tableActions}>
            <span style={{ fontSize: "14px", color: "var(--text-secondary)" }}>共 {currentData.length} 条记录</span>
          </div>
        </div>
                  ...
                  <tr key={role.id}>
                    ...
                    <td>
                      <div className={styles.actionButtons}>
                        <button className={`${styles.actionButton} ${styles.edit}`} title="编辑角色">
                          <FaEdit />
                        </button>
                        <button className={`${styles.actionButton} ${styles.edit}`} title="查看权限">
                          <FaEye />
                        </button>
                        {!role.isSystem && (
                          <button
              ...
              {currentUserPermissions.length > 0 ? (
                currentUserPermissions.map((userPermission) => (
                  <tr key={userPermission.id}>
                    ...
                    <td>
                      <div className={styles.actionButtons}>
                        <button className={`${styles.actionButton} ${styles.edit}`} title="编辑权限">
                          <FaUserCog />
                        </button>
                        <button
        ...
        {totalPages >= 1 && (
          <div className={styles.paginationContainer}>
            <div className={styles.paginationInfo}>
              显示 {startIndex + 1} - {Math.min(endIndex, currentData.length)} 条， 共 {currentData.length} 条记录
            </div>
