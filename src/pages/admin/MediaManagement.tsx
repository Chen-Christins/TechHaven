...
const MediaManagement: React.FC = () => {
  ...
  const downloadFile = (file: MediaFile) => {
    ...
    // 更新下载计数
    setMediaFiles((prev) => prev.map((f) => (f.id === file.id ? { ...f, downloads: f.downloads + 1 } : f)));
  };
    ...
    <div className={styles.mediaManagement}>
      ...
      <div className={styles.pageHeader}>
        <div>
          ...
          <p className={styles.pageDescription}>管理图片、视频、文档等媒体文件</p>
        </div>
      </div>
      ...
      {selectedFiles.length > 0 && (
        <div className={styles.batchActions}>
          <div className={styles.batchInfo}>已选择 {selectedFiles.length} 个文件</div>
          <div className={styles.batchButtons}>
            ...
            <button className={`${styles.btn} ${styles.btnDanger} ${styles.btnSm}`} onClick={() => deleteFiles(selectedFiles)}>
              <FaTrash />
              删除
            </button>
          </div>
      ...
      <div className={styles.statsBar}>
        ...
        <div className={styles.statItem}>
          ...
          <span className={styles.statValue}>{formatFileSize(filteredFiles.reduce((total, file) => total + file.size, 0))}</span>
        </div>
      </div>
                ...
                <div key={file.id} className={styles.fileCard}>
                  ...
                  <div className={styles.filePreview}>
                    {file.type === "image" && file.thumbnailUrl ? (
                      ...
                      <div className={styles.fileIcon}>{getFileIcon(file.type, file.mimeType)}</div>
                    )}
                  </div>
                  <div className={styles.fileInfo}>
                    ...
                    <div className={styles.fileTags}>
                      ...
                      {file.tags.length > 2 && <span className={styles.moreTags}>+{file.tags.length - 2}</span>}
                    </div>
                  </div>
                  <div className={styles.fileActions}>
                    ...
                    <button className={`${styles.actionButton} ${styles.download}`} title="下载" onClick={() => downloadFile(file)}>
                      <FaDownload />
                    </button>
                    <button className={`${styles.actionButton} ${styles.edit}`} title="编辑">
                      <FaEdit />
                    </button>
                    <button className={`${styles.actionButton} ${styles.delete}`} title="删除" onClick={() => deleteFiles([file.id])}>
                      <FaTrash />
                    </button>
                  </div>
                </div>
              ...
              currentItems.map((file) => (
                <div key={file.id} className={styles.listItem}>
                  ...
                  <div className={styles.listName}>
                    <div className={styles.listFileIcon}>{getFileIcon(file.type, file.mimeType)}</div>
                  ...
                  <div className={styles.listActions}>
                    <button className={`${styles.actionButton} ${styles.small}`} title="预览" onClick={() => handlePreviewFile(file)}>
                      <FaEye />
                    </button>
                    <button className={`${styles.actionButton} ${styles.small}`} title="下载" onClick={() => downloadFile(file)}>
                      <FaDownload />
                    </button>
                    <button className={`${styles.actionButton} ${styles.small}`} title="删除" onClick={() => deleteFiles([file.id])}>
                      <FaTrash />
                    </button>
                  </div>
                </div>
        ...
        {totalPages >= 1 && (
          <div className={styles.paginationContainer}>
            <div className={styles.paginationInfo}>
              显示 {startIndex + 1} - {Math.min(endIndex, filteredFiles.length)} 条， 共 {filteredFiles.length} 个文件
            </div>
            ...
            <div className={styles.previewFooter}>
              <div className={styles.previewInfo}>
                ...
                {previewFile.dimensions && (
                  <span>
                    尺寸：{previewFile.dimensions.width}×{previewFile.dimensions.height}
                  </span>
                )}
                {previewFile.duration && <span>时长：{formatDuration(previewFile.duration)}</span>}
                <span>上传者：{previewFile.author.name}</span>
              </div>
              <div className={styles.previewActions}>
                <button className={`${styles.btn} ${styles.btnPrimary}`} onClick={() => downloadFile(previewFile)}>
                  <FaDownload />
                  下载
                </button>
              </div>
