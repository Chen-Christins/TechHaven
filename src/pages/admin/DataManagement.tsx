    ...
    const filteredBackups = useMemo(() => {
        return backups.filter((backup) => {
            if (filters.search) {
                ...
                if (!backup.name.toLowerCase().includes(searchTerm) && !backup.description.toLowerCase().includes(searchTerm)) {
                    return false;
                }
            }
        ...
        await confirm({
            ...
            content: (
                <div>
                    <p>
                        确定要导出
                        {type === "articles" ? "文章" : type === "users" ? "用户" : type === "comments" ? "评论" : "完整"}
                        数据吗？
                    </p>
                </div>
            ),
        ...
        await confirm({
            ...
            content: (
                <div>
                    <p>
                        确定要删除"
                        {(item as BackupRecord).name || (item as ExportRecord).name}
                        "记录吗？
                    </p>
                    <p style={{ color: "var(--error-color)" }}>删除后无法恢复，相关文件也会被删除。</p>
                </div>
            ),
            confirmText: "确认删除",
            cancelText: "取消",
    ...
    const cleanupData = async () => {
        await confirm({
            ...
            content: (
                <div>
                    <p>确定要清理过期数据吗？</p>
                    <p style={{ color: "var(--warning-color)" }}>这将删除30天前的临时文件和失效记录。</p>
                </div>
            ),
            confirmText: "确认清理",
            cancelText: "取消",
                ...
                <>
                    ...
                    <div className={styles.storageSection}>
                        <div className={styles.storageHeader}>
                            ...
                            <button className={`${styles.btn} ${styles.btnOutline} ${styles.btnSm}`} onClick={cleanupData}>
                                <FaBroom />
                                清理数据
                            </button>
                        </div>
                        ...
                        <div className={styles.storageBar}>
                            <div
                                ...
                                style={{
                                    width: `${(stats.usedSize / stats.totalSize) * 100}%`,
                                }}
                            ></div>
                        </div>
                    </div>
                ...
                <>
                    ...
                    <div className={styles.filterSection}>
                        ...
                        <div className={styles.filterForm}>
                            <div className={styles.filterGroup}>
                                <label className={styles.filterLabel}>{activeTab === "backups" ? "搜索备份" : "搜索导出"}</label>
                                <Input
                            ...
                            <div className={styles.filterGroup}>
                                <label className={styles.filterLabel}>{activeTab === "backups" ? "备份类型" : "导出类型"}</label>
                                <CustomSelect
                                    name="类型"
                                    ...
                                    value={
                                        (activeTab === "backups"
                                            ...
                                            : exportTypeOptions.find((option) => option.id === filters.type)) || null
                                    }
                    ...
                    <div className={styles.tableContainer}>
                        <div className={styles.tableHeader}>
                            <h3 className={styles.tableTitle}>{activeTab === "backups" ? "备份记录" : "导出记录"}</h3>
                                                        ...
                                                        <div className={styles.recordInfo}>
                                                            ...
                                                            <div className={styles.recordDetails}>
                                                                ...
                                                                <div className={styles.recordDescription}>
                                                                    {(backup as BackupRecord).description}
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </td>
                                                    ...
                                                    <td>
                                                        <div className={styles.timeInfo}>
                                                            ...
                                                            {(backup as BackupRecord).completedAt ? (
                                                                <div>
                                                                    <FaCheckCircle />{" "}
                                                                    {formatToChinaTime(Number((backup as BackupRecord).completedAt))}
                                                                </div>
                                                            ) : null}
                                                        </div>
                                                    </td>
                        ...
                        {totalPages >= 1 && (
                            <div className={styles.paginationContainer}>
                                <div className={styles.paginationInfo}>
                                    显示 {startIndex + 1} - {Math.min(endIndex, currentData.length)} 条， 共 {currentData.length}{" "}
                                    条记录
                                </div>
