...
function inferProvider(type: ApiType, rawUrl: string): ServiceProvider {
    try {
        ...
        if (type === "openai" && host === "api.openai.com") {
            return "openai";
        }
        if (type === "claude" && host === "api.anthropic.com") {
            return "anthropic";
        }
        if (type === "glm" && host === "open.bigmodel.cn") {
            return "zhipu";
        }
    } catch {
...
function inferResponseType(type: ApiType, rawUrl: string): ResponseType {
    if (/\/responses\/?$/i.test(rawUrl)) {
        return compatibleResponseType(type, "responses");
    }
    if (/\/messages\/?$/i.test(rawUrl)) {
        return compatibleResponseType(type, "messages");
    }
    if (/\/chat\/completions\/?$/i.test(rawUrl)) {
        return compatibleResponseType(type, "chat_completions");
    }
    return defaultResponseType(type);
}

function compatibleResponseType(type: ApiType, value: unknown): ResponseType {
    if (type === "openai" && (value === "responses" || value === "chat_completions")) {
        return value;
    }
    if (type === "claude" && value === "messages") {
        return value;
    }
    if (type === "glm" && value === "chat_completions") {
        return value;
    }
    return defaultResponseType(type);
}
...
function validateUrl(raw: string): string | null {
    ...
    if (parsed.username || parsed.password) {
        return "接口地址不能内嵌用户名或密码";
    }
    if (parsed.search || parsed.hash) {
        return "接口地址不能带查询参数或锚点";
    }
...
function validateApiKey(raw: string): string | null {
    if (raw.length > 8192) {
        return "密钥长度超出合理范围（上限 8192 字符），请核对是否复制了多余内容";
    }
    if (/[*•]/.test(raw)) {
        return "请填入完整密钥：脱敏串（含 * 或 •）无法用于运行";
    }
    if (!/^[\x21-\x7e]+$/.test(raw)) {
        return "密钥只能包含可打印 ASCII 字符，不能含空格或换行";
    }
    return null;
}
...
function validateMaxTokens(raw: string): string | null {
    ...
    if (parsed > 1_000_000) {
        return "最大生成长度超出合理范围（上限 1000000）";
    }
    return null;
}
...
const AgentApiConfigCard: React.FC = () => {
    ...
    useEffect(() => {
        ...
        agentAiConfigService
            ...
            .catch((err) => {
                if (active) {
                    setLoadError(err instanceof Error ? err.message : "Agent 配置服务暂时不可用");
                }
            })
            .finally(() => {
                if (active) {
    ...
    if (loading) {
        return <p role="status">正在加载 Agent 配置…</p>;
    }
    if (loadError)
        return (
            <ErrorState
            ...
            <div className={styles.editCardBody}>
                <div className={styles.editFormGroup}>
                    ...
                    <CustomSelect
                        ...
                        onChange={(option) => {
                            if (option) {
                                handleProviderChange(option.id as ServiceProvider);
                            }
                        }}
                        hideBadge
                        placeholder="请选择服务商..."
                ...
                <div className={styles.editFormGroup}>
                    ...
                    <CustomSelect
                        ...
                        onChange={(option) => {
                            if (option) {
                                handleTypeChange(option.id as ApiType);
                            }
                        }}
                        hideBadge
                        placeholder="请选择协议类型..."
                ...
                <div className={styles.editFormGroup}>
                    ...
                    <CustomSelect
                        ...
                        onChange={(option) => {
                            if (option) {
                                handleResponseTypeChange(option.id as ResponseType);
                            }
                        }}
                        hideBadge
                        placeholder="请选择接口类型..."
                ...
                <div className={styles.editFormGroup}>
                    ...
                    <span className={styles.editHint}>
                        填写完整资源地址，也可使用中转/代理地址。将作为 dsh 环境变量{" "}
                        <span className={styles.editHintCode}>{dsh.baseUrl}</span> 注入，Gateway 会按所选接口类型剥离末尾的
                        /responses、/chat/completions 或 /messages
                    </span>
                </div>
                ...
                <div className={styles.editFormGroup}>
                    ...
                    />
                    <span className={styles.editHint}>
                        {maskedKeyRef.current ? "已保存密钥（脱敏显示），如需修改请重新输入" : "密钥加密存储，仅你可见"}。将作为 dsh
                        环境变量 <span className={styles.editHintCode}>{dsh.key}</span>{" "}
                        注入会话专属子进程，不进入浏览器存储，也不会出现在会话视图或日志中
                    </span>
                </div>
                ...
                <div className={styles.editFormGroup}>
                    ...
                    <Input
                        ...
                        placeholder={`如 ${current.defaultModel}`}
                        maxLength={256}
                        size="large"
                    />
                    <span className={styles.editHint}>
                ...
                <div className={styles.editFormGroup}>
                    ...
                    />
                    <span className={styles.editHint}>
                        对应 dsh 的 reasoningEffort 参数。常见值：minimal / low / medium / high /
                        max（具体支持范围取决于模型）；留空使用模型默认
                    </span>
                </div>
