...
// ---------------------------------------------------------------------------
// AI 配置资产：请求体校验与响应视图（字段名保持与前端 AiConfigParams 契约一致）
// ---------------------------------------------------------------------------
...
function providerFrom(type: AiProviderType, raw: unknown): AiServiceProvider {
  const fallback = type === "claude" ? "anthropic" : type === "glm" ? "zhipu" : "openai";
  const value = raw === undefined || raw === null || raw === "" ? fallback : raw;
  if (value !== "openai" && value !== "anthropic" && value !== "zhipu" && value !== "custom") {
    throw new GatewayError(400, '字段 provider 必须是 "openai" | "anthropic" | "zhipu" | "custom"');
  }
  return value;
}

function responseTypeFrom(type: AiProviderType, raw: unknown): AiResponseType {
  const fallback = type === "claude" ? "messages" : "chat_completions";
  const value = raw === undefined || raw === null || raw === "" ? fallback : raw;
  const valid =
    (type === "openai" && (value === "responses" || value === "chat_completions")) ||
    (type === "claude" && value === "messages") ||
    (type === "glm" && value === "chat_completions");
  if (!valid) throw new GatewayError(400, `字段 response_type 与协议类型 ${type} 不匹配`);
  return value as AiResponseType;
}
...
function parseUpdateConfigBody(body: Record<string, unknown>): UpdateConfigInput {
  ...
  const typeHint = body.type === "claude" || body.type === "glm" ? body.type : "openai";
  ...
  if (body.type !== undefined) {
    ...
    if (body.response_type !== undefined) patch.responseType = responseTypeFrom(body.type, body.response_type);
  }
  ...
  try {
    ...
    if (path === "/v1/ai-configs" || path.startsWith("/v1/ai-configs/")) {
      ...
      if (configMatch) {
        ...
        if (method === "PATCH") {
          const body = jsonObject(await readJsonBody(req));
          const updated = await aiConfigStore.update(id, "user", userId, parseUpdateConfigBody(body));
          sendJson(res, 200, configView(updated));
          return;
        }
        if (method === "DELETE") {
